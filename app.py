...
def load_world_macro_data(parquet_file, config_dict):
    return sync_and_get_metrics(parquet_file, config_dict)
...
def render_momentum_matrix(df_metrics, title):
    ...
    # 🐛 FIX 1: Bulletproof backwards compatibility for Pandas formatting
...
# =======================================================
    ...
    if not df_indices.empty:
        ...
        # 🐛 FIX 2: Enforce the index name for Plotly stability
...
# =======================================================
    ...
    df_master = fetch_market_data_bulk()[1] 
...
# =======================================================
    ...
    if not history_records:
        ...
        for stock, info in history_records.items():
            ...
            if selected_date_str in df_hist_str.index:
                ...
                if row['Pct_Change'] >= price_surge_pct and row['Vol_Surge'] >= vol_multiplier:
                    ...
                    results.append({
                        "Stock": stock, "Sector": info['Sector'], "Close Price": row['Close'],
                        "Volume": row['Volume'], "20D Avg Volume": row['Avg_Volume'],
                        "Volume Surge (x)": row['Vol_Surge'], "Price Change (%)": row['Pct_Change'],
        ...
        if results:
            ...
            st.dataframe(df_results.style.format({"Close Price": "{:,.2f}", "Volume": "{:,.0f}", "20D Avg Volume": "{:,.0f}", "Volume Surge (x)": "{:.2f}x", "Price Change (%)": "{:.2f}%"}), use_container_width=True, height=600)
        else:
    ...
    if not df_history.empty:
        ...
        def render_macro_chart(title, y_col, note_text, color_hex, decimal_places=2):
            if y_col in df_history.columns:
                ...
                # Use strictly 'Date' to avoid the ValueError
