...
def next_page():
    if st.session_state.current_page < 7: # UPGRADED TO 7 PAGES
        st.session_state.current_page += 1
...
# =======================================================
# SAFE ADDITION: DATA ENGINE FOR WORLD & MACRO
# =======================================================
@st.cache_data(ttl=300)
def load_world_macro_data(parquet_file, config_dict):
    """Isolated loader for new sheets to avoid breaking Sheets 1-5."""
    symbol_to_name = {ticker: name for ticker, name in config_dict.items()}
    tickers = list(symbol_to_name.keys())
    
    try:
        hist_df = pd.read_parquet(parquet_file)
        available_cols = [t for t in tickers if t in hist_df.columns]
        hist_df = hist_df[available_cols]
    except Exception:
        hist_df = pd.DataFrame()
...
# =======================================================
# SAFE ADDITION: MOMENTUM MATRIX STYLER
# =======================================================
def get_rank_color(val, max_rank):
    """Dynamic RGB gradient exactly like Sectoral Deep Dive styling."""
    try:
...
with col_title:
    ...
    elif st.session_state.current_page == 3: st.markdown("<h2 style='text-align: center;'>Part 3: Nifty Sectoral Deep Dive</h2>", unsafe_allow_html=True)
    elif st.session_state.current_page == 4: st.markdown("<h2 style='text-align: center;'>Part 4: Sectoral Momentum Matrix</h2>", unsafe_allow_html=True)
    elif st.session_state.current_page == 5: st.markdown("<h2 style='text-align: center;'>Part 5: Price-Volume Action Screener</h2>", unsafe_allow_html=True)
    elif st.session_state.current_page == 6: st.markdown("<h2 style='text-align: center;'>Part 6: Macro Indicators Dashboard</h2>", unsafe_allow_html=True)
    elif st.session_state.current_page == 7: st.markdown("<h2 style='text-align: center;'>Part 7: World Indices Momentum Matrix</h2>", unsafe_allow_html=True)

with col_next:
    if st.session_state.current_page < 7: st.button("Next Screen ➡️", on_click=next_page)
...
if st.session_state.current_page == 1:
    ...
    if df_watchlist.empty:
        ...
        # --- NEW UI LAYOUT: Search Bar Left, Count Right ---
        ...
        # Apply Search Filter before final calculations
        if search_query:
            df_filtered = df_filtered[df_filtered["Stock"].str.contains(search_query, case=False, na=False)]
        ...
        with col_count:
            st.markdown(
                f"<div style='text-align: right; font-size: 16px; font-weight: bold; padding-top: 5px; padding-bottom: 10px;'>"
...
# ==========================================
    ...
    if not df_indices.empty:
        ...
        st.divider()
        render_momentum_matrix(df_indices, "Broad Indices Momentum Ranking Matrix")
    else:
        st.warning("⚠️ Could not load data for Nifty Broad Indices.")

# =======================================================
# PAGE 3: NIFTY SECTORAL MOMENTUM PULSE & DEEP DIVE
# =======================================================
...
# =======================================================
    ...
    if not history_records:
        ...
        all_dates = set()
        for info in history_records.values():
            all_dates.update(info['History'].index.strftime('%Y-%m-%d').tolist())
            
        available_dates = sorted(list(all_dates), reverse=True)
        selected_date_str = st.sidebar.selectbox("Select Target Date:", available_dates)
...
# =======================================================
    ...
    with st.spinner("Fetching Global Macro Data..."):
        _, df_history = load_world_macro_data("macro_master.parquet", macro_config)
        
    if not df_history.empty:
        # Calculate the special Gold to Nifty Ratio
        ...
        st.warning("⚠️ Could not load Macro data. Ensure data_fetcher.py has generated macro_master.parquet.")
...
# =======================================================
    ...
    with st.spinner("Processing World Indices Momentum..."):
        df_world, _ = load_world_macro_data("world_indices_master.parquet", world_index_config)
