...
with col_title:
    titles = {
        ...
        4: "Part 4: Sectoral Momentum", 5: "Part 5: Price-Volume Screener", 6: "Part 6: Macro Indicators",
...
# =======================================================
# MASTER DELTA SYNC ENGINE (Used by Broad, Sector, World, Macro)
# =======================================================
@st.cache_data(ttl=300)
def sync_and_get_metrics(parquet_filename, config_dict):
    """A master engine that reads a specific parquet file, fetches live deltas, and returns fully calculated matrices."""
    ...
    for ticker in merged_df.columns:
        ...
        ret_1y = (current_price / clean_series.iloc[-250] - 1) * 100 if len(clean_series) >= 250 else np.nan
...
# =======================================================
# MOMENTUM MATRIX GENERATOR
# =======================================================
def get_rank_color(rank_str, max_rank):
    """Creates a perfect Red-Yellow-Green gradient CSS injection for any table size."""
    try:
        ...
        if ratio <= 0.5: # Green to Yellow
            r = int(27 + (251 - 27) * (ratio / 0.5))
            g = int(94 + (192 - 94) * (ratio / 0.5))
            b = int(32 + (45 - 32) * (ratio / 0.5))
            color = "white" if ratio < 0.35 else "black"
        else: # Yellow to Red
            ratio2 = (ratio - 0.5) / 0.5
            r = int(251 + (183 - 251) * ratio2)
            g = int(192 + (28 - 192) * ratio2)
            b = int(45 + (28 - 45) * ratio2)
...
def render_momentum_matrix(df_metrics, title):
    """Calculates ranks and renders the colored momentum table."""
    ...
    for col in cols_to_rank:
        ...
        # Create string: "1 (+4.20%)"
        df_display[disp_col] = df_ranks[rank_col].astype(str).apply(lambda x: x.split('.')[0] if x != 'nan' else '-') + \
                               " (" + df_metrics[col].apply(lambda x: f"+{x:.2f}%" if x > 0 else f"{x:.2f}%" if pd.notna(x) else "-") + ")"
        df_display[col + '_raw'] = df_ranks[rank_col]
        
    df_display = df_display.sort_values(by='1W Return (%)_raw').reset_index(drop=True)
    ...
    styled = df_clean.style.applymap(lambda v: get_rank_color(v, max_rank), subset=[c for c in df_clean.columns if 'Momentum' in c])
    st.markdown(f"### 🏆 {title}")
...
if st.session_state.current_page == 1:
    ...
    if df_watchlist.empty:
        ...
        # --- NEW: SEARCH BAR ---
        search_query = st.sidebar.text_input("🔍 Search Stock Symbol:", "").strip().upper()
        ...
        # Apply Search Filter
        if search_query: df_filtered = df_filtered[df_filtered["Stock"].str.contains(search_query)]
        ...
        st.markdown(f"<div style='text-align: right; font-size: 16px; font-weight: bold; padding-bottom: 10px;'>Matches: <span style='color: #4CAF50;'>{filtered_total}</span> / {base_total} (<span style='color: #4CAF50;'>{percentage:.1f}%</span>) of base</div>", unsafe_allow_html=True)
...
# =======================================================
# PAGE 2: BROAD INDICES (Charts + Momentum Matrix)
# =======================================================
    ...
    if not df_indices.empty:
        ...
        st.markdown("**Select Timeframe for Charts:**")
        ...
        with col2:
            ...
            if selected_lines:
                ...
                df_melted = df_normalized.reset_index().melt(id_vars='index', var_name='Index', value_name='Return (%)')
                fig_line = px.line(df_melted, x='index', y='Return (%)', color='Index')
                fig_line.update_layout(xaxis_title="", yaxis_title="Cumulative Return (%)", height=400, legend_title_text="")
                st.plotly_chart(fig_line, use_container_width=True)
        
        st.divider()
        # Render the Rank Matrix
        render_momentum_matrix(df_indices, "Broad Indices Momentum Matrix (Sorted by 1-Week Action)")
...
# =======================================================
    ...
    constituent_symbols = get_constituents(drill_sector)
    df_master = fetch_market_data_bulk()[1] # Just check if it exists
    
    if constituent_symbols and not df_master.empty:
        # We need the watchlist logic, so we bypass to Page 1's cached function
        st.session_state.current_page = 1
    ...
    else: st.warning("Could not load sectoral constituent data.")

# =======================================================
# PAGE 4: SECTORAL MOMENTUM MATRIX
# =======================================================
elif st.session_state.current_page == 4:
    with st.spinner("Processing Sectoral Momentum..."):
        df_sectors, _ = sync_and_get_metrics("nifty_indices_master.parquet", sectoral_config)
    
    if not df_sectors.empty:
        render_momentum_matrix(df_sectors, "Nifty Sectoral Momentum Matrix (Sorted by 1-Week Action)")
    else: st.warning("⚠️ Could not load data.")
...
# =======================================================
    ...
    if st.sidebar.button("🔄 Sync 1D Parquet Data", use_container_width=True):
        def quick_sync(): # Inline helper for manual sync
            try:
                ed = pd.read_parquet("nifty_750_master.parquet")
                ed.index = pd.to_datetime(ed.index).tz_localize(None)
                tkrs = list(ed.columns.levels[0])
            except: return False
            ndf = []
            for t in tkrs:
                try:
            ...
            if ndf:
                ndata = pd.concat(ndf, axis=1)
                fdata = pd.concat([ed, ndata])
                fdata = fdata[~fdata.index.duplicated(keep='last')]
                fdata.to_parquet("nifty_750_master.parquet", engine="pyarrow")
                return True
            return False
            
        with st.spinner("Executing fast sync..."):
            if quick_sync():
                st.sidebar.success("✅ Sync complete!"); st.cache_data.clear(); time.sleep(1); st.rerun()
    ...
    if not history_records: st.warning("⚠️ Market data is empty.")
    else:
        all_dates = set()
        ...
        v_mult = st.sidebar.number_input("Min Volume Surge (x)", min_value=1.0, value=2.0, step=0.5)
        p_pct = st.sidebar.number_input("Min Price Surge (%)", min_value=0.5, value=3.0, step=0.5)
        ...
        for stock, info in history_records.items():
            ...
            if selected_date_str in df_hist_str.index:
                ...
                if row['Pct_Change'] >= p_pct and row['Vol_Surge'] >= v_mult:
                    ac = df_hist_str[(df_hist_str['Pct_Change'] >= p_pct) & (df_hist_str['Vol_Surge'] >= v_mult)]
...
# =======================================================
    ...
    st.sidebar.info("Tracking Global Macro constraints.")
    
    @st.cache_data(ttl=300)
    def fetch_macro_data():
        macro_config = {"DX-Y.NYB": "Dollar Index", "GC=F": "Gold", "CL=F": "Crude Oil", "^TNX": "10Y Bond Yield", "INR=X": "USD INR", "^NSEI": "Nifty"}
        _, df_hist = sync_and_get_metrics("macro_master.parquet", macro_config)
        if not df_hist.empty:
            df_hist['Gold to Nifty Ratio'] = (df_hist['Gold'] * df_hist['USD INR'] * 1.1 / 31.1) / df_hist['Nifty']
        return df_hist

    df_macro = fetch_macro_data()
    
    if not df_macro.empty:
        t_opts = ["1W", "1M", "3M", "6M", "1Y"]
        s_time = st.radio("Timeframe", t_opts, index=4, horizontal=True, label_visibility="collapsed")
        lk_map = {"1W": 6, "1M": 22, "3M": 64, "6M": 126, "1Y": len(df_macro)}
        df_plot = df_macro.tail(lk_map.get(s_time, len(df_macro))).reset_index()

        def plot_macro(title, y_col, color, note, fmt=",.2f"):
            val = df_macro[y_col].iloc[-1]
            st.markdown(f"### {title} : **{val:{fmt}}**")
            fig = px.line(df_plot, x='index', y=y_col)
            fig.update_traces(line_color=color, line_width=2)
            fig.update_layout(xaxis_title="", yaxis_title="", height=300, margin=dict(t=10, b=10))
            st.plotly_chart(fig, use_container_width=True)
            st.caption(f"💡 *Note: {note}*")
            st.divider()

        c1, c2 = st.columns(2)
        with c1: plot_macro("Dollar Index (DX-Y.NYB)", "Dollar Index", "#1E88E5", "When DX-Y increases, Gold decreases and Emerging Market decreases. Check for 100 mark.")
        with c2: plot_macro("US 10 Yr Bond Yield (^TNX)", "10Y Bond Yield", "#D81B60", "For more FIIs the gap between Indian Bond and US Bond should be 5-7%.", ".3f")
        
        c3, c4 = st.columns(2)
        with c3: plot_macro("Gold in USD (GC=F)", "Gold", "#FFB300", "Global Safe Haven Asset.")
        with c4: plot_macro("Crude Oil in USD (CL=F)", "Crude Oil", "#43A047", "Higher crude generally puts pressure on Indian trade deficit.")

        plot_macro("Gold to Nifty Ratio", "Gold to Nifty Ratio", "#8E24AA", "0.6 above go to Stocks, 0.28 below go to Gold.", ".4f")
    else: st.warning("⚠️ Could not load Macro data.")

# =======================================================
# PAGE 7: WORLD INDICES MOMENTUM
# =======================================================
    ...
    if not df_world.empty:
        render_momentum_matrix(df_world, "World Indices Momentum Matrix (Sorted by 1-Week Action)")
    else: st.warning("⚠️ Could not load data.")
